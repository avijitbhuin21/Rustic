...
function closeSessions(tabs) {
  tabs.filter(t => t.kind === 'terminal' && t.terminalSessionId).forEach(t => {
    import('@/state/terminal')
      .then(m => m.useTerminal.getState().closeTerminal(t.terminalSessionId))
      .catch(() => {});
  });
}
...
export const useEditor = create((set, get) => ({
  ...
  closeGroup: (groupId) => {
    ...
    if (group) closeSessions(group.tabs);
    set(s => {
  ...
  openDiff: ({ projectId, filePath, oid = null, title = null }) => {
    if (!projectId || !filePath) return null;
    const groupId = get().activeGroupId;
    const group = get().groups.find(g => g.id === groupId);
    if (!group) return null;
    const id = `d:${projectId}:${oid ?? 'working'}:${filePath}`;
    ...
    const tab = {
      ...
      diff: { projectId, path: filePath, oid },
    };
  ...
  closeTabInGroup: (id, groupId) => {
    ...
    if (!tab) return;
    if (tab.kind === 'terminal' && tab.terminalSessionId) closeSessions([tab]);
    set(s => ({
      groups: (s.groups ?? []).map(g => {
  ...
  closeOthersInGroup: (id, groupId) => {
    set(s => ({
      groups: s.groups.map(g => {
        if (g.id !== groupId) return g;
        closeSessions(g.tabs.filter(t => t.id !== id));
