...
export function WorkflowsPanel() {
  ...
  const loadRepo = useCallback(async () => {
    ...
    try {
      const list = await invoke('list_repo_workflows', { source: repoSource.trim() });
  ...
  const select = async (name) => {
    ...
    try {
      const b = await invoke('get_workflow_body', { name });
  ...
  const create = async () => {
    ...
    try {
      await invoke('create_workflow', { name: newName.trim(), body: '' });
      setNewName('');
      load();
    } catch (e) {
  ...
  const save = async () => {
    ...
    try {
      await invoke('update_workflow', { originalName: originalName || activeName, name: nextName, body });
  ...
  const install = async () => {
    ...
    try {
      await invoke('install_repo_workflows', {
        source: repoSource.trim(),
        paths,
        names,
      });
      toast.success(`Installed ${selectedRepo.length} workflow(s)`);
      setSelectedRepo([]);
      load();
    } catch (e) {
          ...
          <div className="flex h-full flex-col">
            ...
            <div className="flex flex-1 min-h-0">
              <ScrollArea className="w-1/2 border-r border-border">
                <ul className="p-1">
                  {items.length === 0 && (
                    <li className="px-2 py-1.5 text-xs text-muted-foreground">No workflows.</li>
                  )}
                  {items.map((r) => {
              ...
              <ul className="p-1">
                {repoItems.length === 0 && (
                  <li className="px-2 py-1.5 text-xs text-muted-foreground">
                    {repoSource.trim() ? 'No repo workflows.' : 'Enter a source URL and refresh.'}
                  </li>
                )}
                {repoItems.map((s) => {
