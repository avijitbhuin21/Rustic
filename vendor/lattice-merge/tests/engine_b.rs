...
fn disjoint_files_from_both_sides_compose() {
    ...
    assert!(
        result.merged["a.rs"].contains("10"),
        "ours' edit to a.rs lost"
    );
    assert!(
        result.merged["b.rs"].contains("20"),
        "theirs' edit to b.rs lost"
    );
}
...
fn divergent_edits_to_one_line_conflict() {
    ...
    assert!(
        result.conflicts.contains_key("lib.rs"),
        "expected lib.rs conflicted"
    );
    assert!(
        !result.merged.contains_key("lib.rs"),
        "a conflicted file must not be merged"
    );
}
...
fn a_clean_text_merge_that_breaks_a_reference_is_reported_as_hidden() {
    ...
    assert!(
        !result.is_clean(),
        "a hidden conflict must not count as clean"
    );
}
...
fn a_one_sided_delete_is_carried_through() {
    ...
    assert!(
        result.deleted.contains("gone.rs"),
        "deletion lost: {:?}",
        result.deleted
    );
    assert!(
        !result.merged.contains_key("gone.rs"),
        "deleted file must not be merged back"
    );
    assert!(
        result.merged.contains_key("stay.rs"),
        "untouched file must survive"
    );
}
