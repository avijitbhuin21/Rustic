//! Leaf crate for shared low-level IO helpers. Kept dependency-free so any
//! workspace crate can use it without pulling in gix/tree-sitter/etc.
...
mod tests {
    ...
    fn atomic_write_overwrites() {
        let dir =
            std::env::temp_dir().join(format!("rustic-tests-overwrite-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
